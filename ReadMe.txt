https://github.com/okovtun/VBU_221.git
https://www.youtube.com/watch?v=_qb0nIzcEXM&list=PLeqyOOqxeiINUveaBPm5nxiv0F_CFRGve

https://www.cplusplus.com/
https://en.cppreference.com/w/

TODO:
В Solution Pointer добавить проект Split, и в этом проекте решить следующую задачу:
Есть массив из 10 элементов, заполненный случайными числами. 
Необходимо создать еще два массива even и odd минимально возможного размера,
и в массив even скопировать все четные значения, а в массив odd все нечетные значения 
из исходного массива.

TOREAD:
https://www.softelectro.ru/ieee754.html

TODO:
1. В ветке 'separated' вынести все функции в отдельные файлы так, как мы это делали на занятии;
2. От ветки 'master' создать ветку 'templated' и в ней шаблонизировать все фукнции для работы с массивами,
   кроме функции FillRand();
3. От ветки 'templated' создать ветку 'templated_separate' и в ней вынести шаблонные функции
   в отдельные файлы.
4. В журнал прислать ссылку на репозитороий;

DONE:
В проекте Functions\Arrays перегрузить все функции 
для одномерного и двумерного массива типов float, double и char.

DONE:
Написать следующие функции:
	??? FillRand(???);	//Заполняет массив случайными числами					DONE
	??? Print(???);		//Выводит массив на экран								DONE
	??? Sum(???);		//Возвращает сумму элементов массива					DONE
	??? Avg(???);		//Возвращает среднее-арифметическое элементов массива	DONE
	??? minValueIn(???);//Возвращает минимальное значение в массиве				DONE
	??? maxValueIn(???);//Возвращает максимальное значение в массиве			DONE
	??? shiftLeft(???);	//Сдвигает массив на заданное число элементов влево
	??? shiftRight(???);//Сдвигает массив на заданное число элементов вправо
	-----------------------------------------------------------------
	??? Sort(???);		//Сортирует массив в порядке возрастания

DONE:
В проекте Arrays необходимо выполнить следующие действия над массивом:
1. Ввести значения всех его элементов с клавиатуры;
2. Вывести массив на экран;
3. Вывести массив на экран в обратном порядке (от последнего элемента до нулевого);
4. Вычислить сумму элементов массива;
5. Вычислить среднее-арифметическое элементов массива;
6. Найти минимальное т максимально значение в массиве;
---------------------------------------------------
Есть массив из 10ти элементов, заполненный значениями
1	2	3	4	.... 10
Небходимо обеспечить циклический сдвиг массива на заданное число элементов
сначала влево, потом вправо.

===================================================

DONE:
1. https://github.com/okovtun/BV_011/blob/master/ControlStructures/Geometria/Task/Geometria1.txt
2. https://github.com/okovtun/BV_011/blob/master/ControlStructures/Geometria/Task/ChessBoard.jpg
3. https://github.com/okovtun/BV_011/blob/master/ControlStructures/Geometria/Task/HardChess.jpg

DONE:
1. С клавиатуры вводится число, программа вычисляет факториал этого числа;
2. При помщи цикла for возвести число в степень.
   Показатель степени может быть как положительным, так и отрицательным;
3. Вывести на экран таблицу ASCII-символов по 16 символов в строке;
4. Вывести на экран ряд Фибоначчи, до числа, введенного с клавиатуры;
5. Вывести на экран заданное количество чисел из ряда Фибоначчи;
-----------------------------------------------------------------
6. Вывести на экран таблицу умножения;
7. Вывести на экран таблицу Пифагора;
8. Вывести на экран ряд простых чисел до указанного с клавиатуры;

DONE:
1. Пользователь вводит с клавиатуры число. Если введенное число попадает в диапазон 
   от 0 до 10, программа выводит: "Цель поражена", в противном случае - "Вы промахнулись";
2. Написать программу "Калькулятор", который вычисляет выражения вида: 2+3
	2 + 3 = 5;
------------------------
3. Пользователь вводит число с клавиатуры. 
   Программа должна сказать, введенное число - палиндром, или не палиндром.
   12321 - палиндром
   121 - палиндром
   123 - НЕ палиндром
4. Пользователь вводит с клавиатуры номер автобусного билета.
   Программа должна сказать билет счастливый или обычный.
   123123 - счастливый
   123456 - обычный
5. Написать программу 3Dshooter, которая по нажатой клавише выводит на экран DONE
   действие:
   w - вперед
   s - назад
   a - влево
   d - вправо
   Space - прыжок
   Enter - огонь
   Shooter должен быть регистронезависимым и 
   перемещение так же должно работать через клавиши курсора (стрелки).
------------------------------------------------------------------------

1. С клавиатуры вводится число, программа вычисляет факториал этого числа;
2. При помщи цикла for возвести число в степень.
   Показатель степени может быть как положительным, так и отрицательным;
3. Вывести на экран таблицу ASCII-символов по 16 символов в строке;

========================================================================

DONE:
1. Поменять местами две переменные;
2. Используя оператор typeid() выяснить типы следующих значений:
	5;
	5.;
	.5;
	5.f;
	8ULL;
	'+';
3. Что будет выведено на экран:
	int i=3;
	i = ++i + ++i;
	cout << i << endl;
4. Что будет выведено на экран:
	int i=3;
	i = ++i + 1 + ++i*2;
	cout << i << endl;
5. Что будет выведено на экран:
	int i=3;
	i = ++i + 1 + ++i*=2;
	cout << i << endl;
6. Что будет выведено на экран:
	int i = 3;
	i += ++i + 1 + ++i*=2;
	cout << i << endl;
7. Что будет выведено на экран:
	int i=3;
	i *= ++i + 1 + ++i+=2;
	cout << i << endl;



DONE:
1. Вывести в консоль стишок "В лесу родилась Ёлочка".
   Стишок должен быть выровнен по середине экрана,
   и его куплеты должны быть разделены пустой строкой.
2. Изучить теорию из этих файлов:
	https://github.com/okovtun/VBU_221/blob/master/DataTypes/NumericSystems.pdf
	https://github.com/okovtun/VBU_221/blob/master/DataTypes/DataTypes.pdf
3. Решить задачи: https://github.com/okovtun/VBU_221/blob/master/DataTypes/VariablesTask.pdf
   Для решения этих задач понадобится cin.